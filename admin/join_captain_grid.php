...
$columns = array( 
	...
	2 =>'captain_city',
	3 =>'phone',
	4 =>'captain_requested_on'
);

// getting total number records without any search
$sql = "SELECT c.user_id ";
$sql.=" FROM tbl_users c WHERE captain_status='Pending' AND c.is_deleted='0'";
...
if( !empty($requestData['search']['value']) ) {   
	...
	//$sql.=" OR captain_requested_on LIKE '".$requestData['search']['value']."%' ";
...
while( $row=mysqli_fetch_array($query) ) {  // preparing an array
	...
	$nestedData[] = '<span style="margin-left:10px">'.$row["user_id"].'</span>';
	$nestedData[] = '<span style="margin-left:10px">'.$row["captain_name"].'</span>';
	$nestedData[] = '<span style="margin-left:05px">'.$row["captain_city"].'</span>';
	$nestedData[] = '<span style="margin-left:05px">'.$row["phone"].'</span>';
	$nestedData[] = '<span style="margin-left:5px">'.datFormat($row["captain_requested_on"]).'</span>';
	...
	$statusAction =	'<br><a  href="join_captain_list.php?id='.$encodedId.'&action='.$converter->encode("approveCaptain").'" style="text-decoration:underline" >Approve</a>&nbsp;|
	&nbsp;<a  href="join_captain_list.php?id='.$encodedId.'&action='.$converter->encode("rejectCaptain").'" style="text-decoration:underline" >Reject</a>';
	else if($row["captain_status"]=='Approved')
	$statusAction =	'<br><a  href="join_captain_list.php?id='.$encodedId.'&action='.$converter->encode("rejectCaptain").'" style="text-decoration:underline" >Reject</a>';	
	else
	$statusAction ='';
