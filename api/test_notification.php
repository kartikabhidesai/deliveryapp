<?php
	...
	$postdata1['job_id'] = '0';
	$postdata1['user_id'] = '0';
	$postdata1['message'] = 'Good Morning...';
	$postdata1['created_on'] = time();
	$postdata1['updated_on'] = time();
	$postdata1['image'] = $_POST['image'];
	
	$device_token = $_POST['device_token'];
	
	if($_POST['type']=='ios' || $_POST['type']=='iphone')
		$value = sendToIphone_test($device_token,$postdata1);
		
	else	
		$value = sendToAndroid_test($device_token,$postdata1);
		
		print_R($value);
		
		
	function sendToIphone_test($device_token,$data)
{
...
function sendToAndroid_test($regid,$data)
{
	$dbConn = mysqli_connect(DB_HOST, DB_NAME, DB_PASS, DB_NAME) or die("Connection failed: " . mysqli_connect_error());
	$noti_array = array("add_job_en"=>"You have a new request from ##UNAME##",
						"add_job_ar"=>"##UNAME## لديك طلب جديد من ",
